...
```{r}
# lm_diabetes = lm(formula = carbs ~ age + weight + protein, data=df)
# summary(lm_diabetes)
```
...
Consistent with equation (6) we define 
...
tvbound
```
The following is function that generates $\sigma^2_{n+1}$ given $\sigma^2_n$, equation 7.
...
```{r}
# df <- data.frame(X,Y, it=1:100)
# df %>%
#   pivot_longer(c("X","Y"), names_to = "sim", values_to = "val")
```


```{r}
# df <- data.frame(X,Y, iter_no=1:100)
# df %>%
#   pivot_longer(c("X","Y"), names_to = "sim_no", values_to = "val") %>%
#   ggplot(aes(x = iter_no, y = val)) +
#   geom_line(aes(color = sim_no)) + theme(legend.position = "none") +
#   labs(title = "1 simulation of |X_n-Y_n|", subtitle = "Using common random number technique") +
#   xlab("iteration") + ylab("diff = |X_n-Y_n|")
```
...
#   geom_line(aes(color = sim_no)) 
```
# Autoregressive process

Define the autorregressize process to be 
